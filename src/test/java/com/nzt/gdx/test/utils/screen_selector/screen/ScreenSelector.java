 ...
 */
public class ScreenSelector extends SimpleScreen<ScreenSelectorTestMain> {

    private final NzStage stage;
    private final Skin skin = new Skin(Gdx.files.internal(STryScene2DConstants.UI_SKIN));

    public ScreenSelector(ScreenSelectorTestMain main, CaseST rootCaseTest) {
        super(main);
        stage = new NzStage();
        ...
        InputListener listener = new InputListener() {
            ...
            public boolean touchDown(InputEvent event, float x, float y, int pointer, int button) {
                try {
                    ...
                    if (caseTest.isGroup) {
                        ScreenSelector groupScreen = new ScreenSelector(main, caseTest);
